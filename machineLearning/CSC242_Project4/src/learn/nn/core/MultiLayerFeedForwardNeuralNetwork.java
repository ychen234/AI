...
abstract public class MultiLayerFeedForwardNeuralNetwork extends FeedForwardNeuralNetwork {
	...
	public void backprop(Example example, double alpha) {
		...
		LogisticUnit[] outputs = (LogisticUnit[]) this.getOutputUnits();

	        for (int j = 0; j < outputs.length; j++) {
	            LogisticUnit unit = outputs[j];
	            double gPrime = unit.activationPrime(unit.getInputSum()); // g'(in_j)
	            double y_j = example.outputs[j];
	            double a_j = unit.getOutput();
	            this.layers[this.layers.length-1][j].delta = gPrime * (y_j - a_j); // delta[j];
	        }
	        
	        for (int l = this.layers.length - 2; l > 0; l--) {
	            for (int i = 0; i < this.layers[l].length; i++) {
	                LogisticUnit unit = (LogisticUnit) this.layers[l][i]; // Hidden layers are all logistic
	                double gPrime = unit.activationPrime(unit.getInputSum()); // g'(in_i)

	                double sum = 0;
	                for (Connection c : unit.outgoingConnections) {
	                    sum += (c.weight * c.dst.delta);
	                }

	               this.layers[l][i].delta = gPrime * sum;
	            }
	        }
	        ...
	        for (int l = 1; l < this.layers.length; l++) {
	            ...
	            for (int i = 0; i < units.length; i++) {
	                NeuronUnit unit = units[i];
	                for (Connection conn : unit.incomingConnections) {
	                    double w = conn.weight + alpha * conn.src.getOutput() * conn.dst.delta;
	                    conn.weight = w;
	                }
	            }
	        }
